...
## This is an island compiler, where the implementation used 2 different domain polygons for resolving compartmental flows. 
## This means there is a My_scale, and an Accurate_scale (currently used for freshwater inputs)
...
Accurate_scale <- readRDS("./Objects/Domains-accurate.rds") %>%                # Calculate the volume of the three zones
  sf::st_drop_geometry() %>% 
  mutate(S = T,
         D = case_when(Shore == "Inshore" ~ F,
                       Shore == "Offshore" ~ T)) %>% 
...
Physics_new <- mutate(Physics_template, SLight = My_light$Measured,
                     ...
                     ## River inflow,
                     Rivervol_SI = My_Rivers$Runoff / filter(Accurate_scale, Shore == "Inshore")$Volume, # Scale as proportion of inshore volume
